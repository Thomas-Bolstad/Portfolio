...
# Version 4.0.2 19/05/2010 *Thomas M. B.
#     * Modified the code so that ending a program with an open
#       GraphWin will close the window and return an error
#       if it is ended while performing a CheckMouse
#     * Added capability of graphobjects to handle rgb codes
#       put in as lists (perhaps tuples are better?).
...
class GraphWin(tk.Canvas):
    ...
    def setBackground(self, color):
        ...
        if type(color) == str:
           self.config(bg=color)
            ...
            for i in color:
                if not 0 <= i <= 255:
                    raise Exception("Faulty values of rgb colors.")
            self.config(bg=color_rgb(color[0],color[1],color[2]))
        self.__autoflush()
   ...
   #Thomas B: This is one of the places, where I've allowed for using rgb
   #          colors, as well as colors with string names. This code with
   #          small modifications is repeated at the different functions
   #          which takes color arguments.
        ...
        if type(color) == str:
           self.create_line(xs,ys,xs+1,ys, fill=color)
            ...
            for i in color:
                if not 0 <= i <= 255:
                    raise Exception("Faulty values of rgb colors.")
            self.create_line(xs,ys,xs+1,ys,fill=color_rgb(color[0],color[1],color[2]))
        self.__autoflush()
    ...
    def setFill(self, color):
        ...
        if type(color) == str:
            ...
            for i in color:
                if not 0 <= i <= 255:
                    raise Exception("Faulty values of rgb colors.")
            self._reconfig("fill", color_rgb(color[0],color[1],color[2]))
    ...
    def setOutline(self, color):
        ...
        if type(color) == str:
            ...
            for i in color:
                if not 0 <= i <= 255:
                    raise Exception("Faulty values of rgb colors.")
            self._reconfig("outline", color_rgb(color[0],color[1],color[2]))
        ...
        if self.entry:
            if type(color) == str:
                ...
                for i in color:
                    if not 0 <= i <= 255:
                        raise Exception("Faulty values of rgb colors.")
                self.entry.config(fill=color_rgb(color[0],color[1],color[2]))
